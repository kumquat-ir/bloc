...
proc initCamera*(winw, winh: cint, ipos: Vec3[GLfloat]): Camera =
...
proc handleKbd*(cam: var Camera, kbd: ptr array[0..512, uint8]) =
...
proc mat3[T](m4: Mat4[T]): Mat3[T] =
  return mat3(vec3(m4[0][0], m4[0][1], m4[0][2]),
              vec3(m4[1][0], m4[1][1], m4[1][2]),
              vec3(m4[2][0], m4[2][1], m4[2][2]))

proc rotate[T](vec: Vec[3, T], angle: SomeFloat, normal: Vec[3, T]): Vec[3, T] =
  return mat3[T](rotate(mat4[T](1), angle, normal)) * vec

proc angle[N, T](a: Vec[N, T], b: Vec[N, T]): T =
  return arccos(clamp(dot(a, b), T(-1), T(1)))

proc handleMouse*(cam: var Camera, relx, rely: int32) =
  ...
  if abs(angle(vorient, cam.up) - radians(90.0)) <= radians(89.0):
    cam.orient = vorient
